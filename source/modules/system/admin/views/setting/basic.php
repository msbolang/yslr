...
<?php $form = ActiveForm::begin(); ?>
   ...
   <?= $form->field($model, 'sys_site_name_en')?>
    <?= $form->field($model, 'sys_site_description')?>
   <?= $form->field($model, 'sys_site_description_en')?>
    ...
    <?= $form->field($model, 'sys_site_about_en',['size'=>'default'])->textarea()?>
    <?= $form->field($model, 'sys_status')->radioList(['1'=>'正常','0'=>'关闭'])?>
    <?php echo KindEditor::widget(['inputId'=>'#basicconfig-sys_site_about','width'=>'70%'])?>
   <?php echo KindEditor::widget(['inputId'=>'#basicconfig-sys_site_about_en','width'=>'70%'])?>
    <?= $form->defaultButtons() ?>
<?php ActiveForm::end(); ?>
